...
static tInfo *_users = NULL;
static tInfo *_groups = NULL;
static int _usersSize = 0;
static int _groupsSize = 0;

static tInfo *add_element_in_table(tInfo **tb, int *tbSize, char *name, int id)
{
	DEBUG((MYLOG_DEBUG, "add_element_in_table(%p, %i, %s, %i)", *tb, *tbSize, name, id));
	if ((*tbSize % TB_ALLOC_SIZE) == 0)
	{
		*tb = realloc(*tb, (*tbSize + TB_ALLOC_SIZE) * sizeof(tInfo));

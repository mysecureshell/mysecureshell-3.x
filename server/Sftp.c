...
void DoRealPath()
{
	...
	path = EncodingFromUtf8(BufferGetString(bIn), 1);
	resolvePath = FSRealPath(path);
	memset(&s, 0, sizeof(s));
	if (cVersion >= 4)
		s.name = EncodingToUtf8(resolvePath->exposedPath, 0);
	else
	{
...
void DoReadDir()
{
	...
	else
	{
		...
		{
			EncodeStatToAttributes(&st, &(s[count].attributes), path->realPath);
			s[count].name = EncodingToUtf8(path->path, 0);
			if (cVersion <= 3)
...
void DoStat(int doLStat)
{
	...
	{
		EncodeStatToAttributes(&st, &a, path);
		if (cVersion >= 4)
			a.flags = flags;
...
void DoSetStat(int usePath)
{
	...
	else if (resolvedPath != NULL)
	{
		...
		{
			if (utimes(resolvedPath->realPath, EncodeAttributesToTimeval(a)) == -1)
				status = errnoToPortable(errno);
		}
...
void DoRename()
{
	...
	id = BufferGetInt32(bIn);
	oldPath = EncodingFromUtf8(BufferGetString(bIn), 1);
	newPath = EncodingFromUtf8(BufferGetString(bIn), 1);
	if (cVersion >= 5)
		flags = BufferGetInt32(bIn);
...
void DoSFTPProtocol()
{
	...
	DEBUG((MYLOG_DEBUG, "[DoSFTPProtocol]connectionStatus:%i msgType:%i msgLen:%i", connectionStatus, msgType, msgLen));
...
int SftpLoop(tGlobal *params, int sftpProtocol, int clientFd)
{
	...
	for (;;)
	{
		...
		printf("SftpMain [clientFd:%i] tv_sec=%li tv_usec=%li\n", clientFd, tm.tv_sec, tm.tv_usec);
		//DEBUG((MYLOG_DEBUG, "[select: %i]tmLast: %lli tmCur: %lli tmNeeded: %lli", ret, tmLast, tmCur, tmNeeded));
		if ((ret = select(clientFd + 1, &fdR, &fdW, NULL, &tm)) == -1)
		{
			if (errno != EINTR)
				return (1);
		}
...
printf("SftpMain ret=%i\n", ret);
		...
		{
printf("SftpMain ret=%i canRead=%i canWrite=%i\n", ret, FD_ISSET(clientFd, &fdR) ? 1 : 0, FD_ISSET(clientFd, &fdW) ? 1 : 0);
			if (gl_var->who != NULL)
				gl_var->who->time_idle = 0;
			if (FD_ISSET(clientFd, &fdR))
			{
				u_int32_t todo;
				...
				len = read(clientFd, BufferGetWritePointer(bIn), todo);
				if (len < 0)
					return (1);
				else if (len == 0)
					return (1);
			...
			if (FD_ISSET(clientFd, &fdW))
			{
				...
				len = write(clientFd, BufferGetReadPointer(bOut), todo);
				if (len < 0)
					return (1);
				else
